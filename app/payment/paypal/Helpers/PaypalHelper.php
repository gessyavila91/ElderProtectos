...
class PaypalHelper {
    ...
    public function __construct() {
        ...
        $this->_apiUrl = config('payment.paypal.PAYPAL_ENDPOINTS.'.config('payment.paypal.PAYPAL_ENVIRONMENT'));
    }
    ...
    private function _setDefaultHeaders() {
        $this->_http->addHeader("PayPal-Partner-Attribution-Id: " . config('payment.paypal.SBN_CODE'));
    }
    ...
    private function _createApiUrl($resource) {
        if($resource == 'oauth2/token')
            ...
            return $this->_apiUrl . "/" . config('payment.paypal.PAYPAL_REST_VERSION') . "/" . $resource;
    }
    ...
    private function _getToken() {
        ...
        $this->_http->setAuthentication(
            config('payment.paypal.PAYPAL_CREDENTIALS.sandbox.client_id').':'.config('payment.paypal.PAYPAL_CREDENTIALS.sandbox.client_secret')
        );
